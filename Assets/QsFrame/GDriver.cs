    ...
    void Start()
    {
        ...
        StartCoroutine(ieCheckResources(()=>
            StartCoroutine(ieUpdateResources(() =>
                {
                    uiRes.ShowTips("更新完成，正在初始化......");
                    //游戏初始化
                    GApp.Init();
                    GApp.Reset();

                    GApp.LuaMgr.LoadLua("main");
        ...
        if (File.Exists(localManifestPath))
        {
            ...
            for (int i = 0; i < allNewAssetBundles.Length; i++)
            {
                if (allLocalAssetBundles.Contains(allNewAssetBundles[i]))
                {
                    ...
                    {
                        Debug.Log(string.Format("校验资源名称： {0}  HashID：old-{1}  new-{2}", allNewAssetBundles[i], localHashID,newHashID));
                        lUpdateFile.Add(allNewAssetBundles[i]);
                    }
                }
                else
                {
                    Debug.Log(string.Format("校验资源名称： {0}  HashID：{1}", allNewAssetBundles[i], newManifest.GetAssetBundleHash(allNewAssetBundles[i])));
                    lUpdateFile.Add(allNewAssetBundles[i]);
                }
            }
        }
